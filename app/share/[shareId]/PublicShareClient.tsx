...
type PublicNote = Note & { user: { name: string } };

export default function PublicShareClient({ note }: { note: PublicNote }) {
    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 py-16">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}
          ...
          <span className="text-sm" style={{ color: "var(--text-muted)" }}>By {note.user.name}</span>
        ...
        <div className="mt-16 pt-8" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <p className="text-xs text-center" style={{ color: "var(--text-muted)" }}>
            Shared via <a href="/" className="text-violet-400 hover:text-violet-300">Peblo Notes</a>
          </p>
        </div>
      </div>
    </div>
